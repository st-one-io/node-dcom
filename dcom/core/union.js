...
class Union {
    ...
    constructor(discriminantClass) {
        ...
        if (!(discriminantClass === 'Integer' ||
            discriminantClass === 'Short' ||
            discriminantClass === 'Boolean' ||
            discriminantClass === 'Character')) {
            throw new Error('UNION_INCORRECT_DISC' +
             new ErrorCodes().UNION_INCORRECT_DISC);
        }
    ...
    addMember(discriminant, member) {
        ...
        if (!discriminant || !member) {
            throw new Error('UNION_NULL_DISCRMINANT' +
             new ErrorCodes().UNION_NULL_DISCRMINANT);
        }

        if (!discriminant.getClass().equals(discriminantClass)) {
            throw new Error('UNION_DISCRMINANT_MISMATCH' +
             new ErrorCodes().UNION_DISCRMINANT_MISMATCH);
        }

        if ((member instanceof Pointer &&
            !member.isReference()) || member instanceof ComString) {
    ...
    encode(ndr, listOfDefferedPointers, flags) {
        if (this.dsVsMember.size != 1) {
            throw new Error('UNION_DISCRMINANT_SERIALIZATION_ERROR' +
                new ErrorCodes().UNION_DISCRMINANT_SERIALIZATION_ERROR);
        }

        // first write the discriminant and then the member
        let keys = this.dsVsMember.keys();
        MarshalUnMarshalHelper.serialize(ndr, this.discriminantClass,
            keys.next().value, listOfDefferedPointers, flags);
        ...
        if (!value.equals(Struct.MEMBER_IS_EMPTY)) {
            MarshalUnMarshalHelper.serialize(ndr,
                MarshalUnMarshalHelper.inferClass(value), value,
    ...
    decode(ndr, listOfDefferedPointers, flags, additionalData) {
        ...
        if (this.dsVsMember.size == 0) {
            throw new Error('UNION_DISCRMINANT_DESERIALIZATION_ERROR' +
                new ErrorCodes().UNION_DISCRMINANT_DESERIALIZATION_ERROR);
        }
        ...
        let key = MarshalUnMarshalHelper.deSerialize(ndr,
            this.discriminantClass, listOfDefferedPointers,
            flags, additionalData);
        ...
        if (value != (Struct.MEMBER_IS_EMPTY)) {
            retVal.dsVsMember.set(key, 
                MarshalUnMarshalHelper.deSerialize(ndr, value,
    ...
    getLength() {
        ...
        for (const o of this.dsVsMember.keys()) {
            let temp = MarshalUnMarshalHelper.getLengthInBytes(
                MarshalUnMarshalHelper.inferClass(o), o,
                Flags.FLAG_NULL);
            length = Math.max(length, temp); // length of the largest member
        }

        return length + MarshalUnMarshalHelper.getLengthInBytes(
            this.discriminantClass, null, Flags.FLAG_NULL);
    }
