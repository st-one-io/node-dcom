...
/**
 * OrpcThis class. Check the documentation for specifics
 * about this object. The important here is that it must
...
class OrpcThis {
  ...
  decode(ndr) {
    ...
    let map = new Map();
    let majorVersion = MarshalUnMarshalHelper.deSerialize(
        ndr, new ComValue(null, types.SHORT), null, Flags.FLAG_NULL, map);
    let minorVersion = MarshalUnMarshalHelper.deSerialize(
        ndr, new ComValue(null, types.SHORT), null, Flags.FLAG_NULL, map);

    retval.version = new ComVersion(majorVersion, minorVersion);
    retval.flags = MarshalUnMarshalHelper.deSerialize(
        ndr, new ComValue(null, types.INTEGER), null, Flags.FLAG_NULL, map);

    MarshalUnMarshalHelper.deSerialize(
        ndr, new ComValue(null, types.INTEGER),
        null, Flags.FLAG_NULL, map);// reserved.
    ...
    orpcextent.addMember(new ComValue(new ComArray(
        new ComValue(null, types.BYTE), null, 1, true), types.COMARRAY));
    ...
    let comOrpcExtentPointer = new ComValue(
        new Pointer(comOrpcExtent), types.POINTER);
    let comOrpcExtentPointerArray = new ComValue(
        new ComArray(comOrpcExtentPointer, null, 1, true), types.COMARRAY);
    let comOrpcExtentPointerArrayPointer = new ComValue(
        new Pointer(comOrpcExtentPointerArray), types.POINTER);
    ...
    let comOrpcextentarrayPointer = new ComValue(
        new Pointer(comOrpcextentarray), types.POINTER);
    /** @type {Pointer[]} */
    let listOfDefferedPointers = [];
    let orpcextentarrayptr = MarshalUnMarshalHelper.deSerialize(ndr,
        comOrpcextentarrayPointer, listOfDefferedPointers,
        Flags.FLAG_NULL, map);
    let x = 0;

    while (x < listOfDefferedPointers.length) {
      let newList = [];
      let replacement = MarshalUnMarshalHelper.deSerialize(ndr,
          new ComValue(listOfDefferedPointers[x], types.POINTER),
          newList, Flags.FLAG_NULL, map);
